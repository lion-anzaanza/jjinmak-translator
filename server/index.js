...
const CONSECUTIVE_THRESHOLD = 5; // 연속 N번 기준 미만이면 차단
...
app.post('/api/translate', translateLimiter, (req, res) => {
  ...
  // VPN IP 로테이션 감지 (이름별 고유 대역 추적)
  const ip = req.ip.replace('::ffff:', '');
  const subnet = ip.split('.').slice(0, 2).join('.');
  const subnets = nameSubnets.get(trimmedName) || new Set();
  subnets.add(subnet);
  nameSubnets.set(trimmedName, subnets);

  if (subnets.size >= VPN_SUBNET_THRESHOLD) {
    macroBanned.set(req.ip, trimmedName);
    fs.promises.writeFile(bannedIPsPath, JSON.stringify([...macroBanned].map(([ip, name]) => ({ ip, name })), null, 2)).catch(e => console.error('bannedIPs 저장 실패:', e.message));
    console.log(`[VPN영구차단] ${trimmedName} | 고유 대역 ${subnets.size}개 | IP: ${req.ip}`);
    return res.status(429).json({ error: '이상한 짓 하지 마래이! VPN 쓰면 다 보인데이!' });
  }
